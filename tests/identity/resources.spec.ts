...
test('creates, versions, pins and deletes a private text resource', async ({ page, browser }) => {
  ...
  await page.getByLabel('제목', { exact: true }).fill('레이스 회복 메모');
  await expect(page.getByRole('combobox', { name: '분류' })).toHaveValue('note');
  await page.getByLabel('원문', { exact: true }).fill('첫 원문 문단입니다.\n\n두 번째 문단입니다.');
  await page.getByRole('button', { name: 'private 자료 저장' }).click();
  ...
  const list = privateTextResourceListSchema.parse(await listResponse.json());
  ...
  try {
    ...
    expect(privateTextResourceListSchema.parse(await otherList.json()).items).toEqual([]);
  } finally {
    await otherContext.close();
  }

  const versionForm = page.getByRole('form', { name: '새 자료 버전 저장' });
