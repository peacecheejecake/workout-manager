...
it('exports coaching records only while evidence and AI consent remain available', async () => {
  ...
  if (before.schemaVersion !== 22) throw new Error('Expected coaching export v22');
  ...
  if (otherExport.schemaVersion !== 22) throw new Error('Expected coaching export v22');
  ...
  if (withdrawnArtifact.schemaVersion !== 22) throw new Error('Expected coaching export v22');
  ...
  if (guarded.schemaVersion !== 22) throw new Error('Expected coaching export v22');
...
it('exports pending create URL ingestion history before a resource row exists', async () => {
  ...
  const exported = await operations.exportAccount(athlete);
  if (exported.schemaVersion !== 22) throw new Error('Expected the current account export schema.');
...
it('removes every URL-derived collection from export after the resource is soft deleted', async () => {
  ...
  const before = await operations.exportAccount(athlete);
  if (before.schemaVersion !== 22) throw new Error('Expected the current account export schema.');
  ...
  if (deleted.schemaVersion !== 22) throw new Error('Expected the current account export schema.');
